...
def collate_fn(batch):
    ...
    for a,b,c,d in batch:
        ...
        seq_len = ceil(audio.shape[-1]/16000*50) #(1000/20)
        max_seq_len = max(max_seq_len, seq_len)
    max_seq_len = min(1500, max_seq_len)
    whisper_noisy = torch.stack(whisper_noisy)[:,:max_seq_len,:]
    whisper_clean = torch.stack(whisper_clean)[:,:max_seq_len,:]

    plbertembs, attention_mask = get_pltbert_embs(raw_sents, global_phonemizer, tokenizer, text_cleaner)
    ...
    return plbertembs.cpu().detach(), whisper_noisy.cpu().detach(), speakerembs.cpu().detach(), whisper_clean.cpu().detach(), attention_mask.cpu().detach()
